async function testDrago() {
  const companyName = "Drago";

  function normalizeCompanyName(name) {
    let cleaned = name.replace(/s\.?p\.?a\.?|s\.?r\.?l\.?|llc|inc\.?|ltd\.?|gmbh/gi, '').trim();
  ...
  console.log("--- 1. Testing LinkedIn ---");
  ...
  try {
    ...
    if (res.ok) {
      ...
      else console.log("LinkedIn sameAs: Not found");
    } else {
